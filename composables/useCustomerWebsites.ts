import type { Website, Filter } from '../types'
...
export const useUseCustomerWebsites = () => {
  ...
  const website = useState<Website>('website', () => ({} as Website))
  ...
  async function fetchList() {
    ...
    const res = await $fetch<{ websites: Website[] }>('https://api.nuxt.com/modules')
    if (res?.websites) {
  ...
  const filteredWebsite = computed<Website[]>(() => {
    let filteredWebsite = [...websites.value]
      .filter((website: Website | any) => {
      ...
      .sort((a: Website, b: Website) => b.stats[selectedSort.value.key] - a.stats[selectedSort.value.key])

    if (selectedOrder.value.key === 'asc') {
      filteredWebsite = filteredWebsite.reverse()
    }

    // sponsored modules in first place
    return filteredWebsite.sort(isSponsor)
  })
