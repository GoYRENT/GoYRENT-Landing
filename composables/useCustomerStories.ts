import type { StoriesList, Filter, StoriesListGroupItem } from '../types'


export const useCustomerStories = () => {
  const route = useRoute()
  const router = useRouter()
  const storiesList = useState<StoriesList>('showcase', () => null)
  ...
  async function fetchList() {
    if (storiesList.value) {
      return
    }

    const res = await $fetch<StoriesList>('https://api.nuxt.com/showcase')
    ...
    storiesList.value = res
  }

  // Lists
  const categories = computed<Filter[]>(() => {
    return storiesList.value?.groups?.map(group => ({
      key: group.id,
      label: group.name,
      exact: true,
      exactQuery: true,
  ...
  const selectedShowcases = computed<ShowcaseListGroupItem[]>(() => {
    ...
    return storiesList.value?.groups
