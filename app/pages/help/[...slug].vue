...
const { data: page } = await useAsyncData(routePath.value, () => queryCollection('help').path(routePath.value).first())
if (!page.value) {
  throw createError({ statusCode: 404, statusMessage: 'Page not found', fatal: true })
}

const { data: surround } = await useAsyncData(`${routePath.value}-surround`, () => {
  return queryCollectionItemSurroundings('help', routePath.value, {
    fields: ['description']
  })
}, { default: () => [] })
...
defineOgImage('OgImageSaas', { title, description, headline: 'Help' })
</script>
